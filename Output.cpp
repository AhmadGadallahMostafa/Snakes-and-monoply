...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	int y = (cellPos.VCell()+1) *(UI.CellHeight);
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_DICE_VALUE] = "images\\Add_Dice_Card.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\New_Game.jpg";
	MenuItemImages[ITM_EXIT2] = "images\\Menu_Exit2.jpg";
	//Gadallah:add exit  w add el add dice value w ma tnsa4 t7ot l swr .
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum > 3 || playerNum < 0 || !cellPos.IsValidCell())
		return;
	...
	pWind->DrawCircle(x,y,radius);//Moaz:[8yr 2l styles zy mnta 3ayz w lw mktbtsh style by5leha lw7do filled] (comment 1)
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int yy = y1-0.25*UI.CellHeight;
	int arrx[4] = { x12,x12 - xChange,x12,x12 + xChange };
	int arry[4] = { yy,yy+ yChange,yy + 2 * yChange,yy + yChange };
