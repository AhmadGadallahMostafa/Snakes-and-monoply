...
#include <fstream>
CardEleven::CardEleven(CellPosition const &pos) : Card(pos)
{
	cardNumber = 11;
}
...
void CardEleven::ReadCardParameters(Grid *pGrid)
{
	...
	if (price == 0 && fees == 0)
	{
		pOut->PrintMessage("New CardEleven: Please enter price of station");
...
void CardEleven::Save(ofstream& outFile, int num)
{
	if (num == 3)
	{
		outFile << "11" << " " << position.GetCellNumFromPosition(position) << " " << price << " " << fees << endl;
...
void CardEleven::Load(ifstream& InFile, Grid* pGrid)
{
	...
	CardEleven* PC11 = new CardEleven(CellN);
	PC11->price = price; PC11->fees = fees;
	pGrid->AddObjectToCell(PC11);
}
