...
public:
	CardOne(const CellPosition & pos); // A Constructor takes card position
	 ...
	 void ReadCardParameters(Grid * pGrid); // Reads the parameters of CardOne which is: walletAmount

	 void Apply(Grid* pGrid, Player* pPlayer); // Applies the effect of CardOne on the passed Player
